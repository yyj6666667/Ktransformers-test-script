...
def get_model_configs():
    ...
    C.append(("Qwen3-Coder-Next", "Qwen3-Coder-Next", {
        ...
        "kt-cpuinfer": 16, "kt-threadpool-count": 1,
        "kt-num-gpu-experts": 12, "kt-method": "BF16",
        "kt-gpu-prefill-token-threshold": 2048,
        "attention-backend": "triton",
    ...
    C.append(("Qwen3-Coder-Next-FP8", "Qwen3-Coder-Next-FP8", {
        ...
        "kt-cpuinfer": 16, "kt-threadpool-count": 1,
        "kt-num-gpu-experts": 20, "kt-method": "FP8",
        "kt-gpu-prefill-token-threshold": 2048,
        "attention-backend": "triton",
        "fp8-gemm-backend": "cutlass",
        "trust-remote-code": True, "mem-fraction-static": 0.80,
            ...
            with open("/etc/os-release", "r") as f:
                for line in f:
                    if line.startswith("PRETTY_NAME="):
                        os_description = line.split("=", 1)[1].strip().strip('"')
                        break
        except Exception:
            pass
...
def print_hardware_info(hw_info):
    ...
    print(f"    OS:     {plat.get('os_description', 'N/A')}")
    if plat.get("is_wsl"):
        print("    WSL:    Yes")
...
def save_results_csv(all_results, filepath, hw_info=None):
    ...
    with open(filepath, "w", newline="") as f:
        ...
        if hw_info:
            ...
            writer.writerow([f"# OS: {plat.get('os_description', 'N/A')}"])
            writer.writerow([f"# Kernel: {plat.get('release', 'N/A')}"])
            if plat.get("is_wsl"):
                writer.writerow(["# WSL: Yes"])
